    ...
    def do_GET(self):
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                print IDPath
                restaurant = session.query(Restaurant).filter_by(id=IDPath).one()
                if restaurant:
                    self.send_response(200)
                    self.send_header('Content-type', 'text/html')
                    self.end_headers()
            ...
            if self.path.endswith("/delete"):
                ...
                if restaurant:
                    ...
                    output += "<input type = 'submit' value = 'Yes, I never want to see it again...'>"
                    output += "</form>"
                    output += "</body></html>"
            ...
            if self.path.endswith("/edit"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    restaurants = session.query(Restaurant).filter_by(
                        id=IDPath).one()
                    if restaurants != []:
                        restaurants.name = messagecontent[0]
